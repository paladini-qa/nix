  ...
  if (transactions.length === 0) {
    return (
      <div className="bg-white dark:bg-white/5 rounded-xl shadow-sm border border-gray-100 dark:border-white/10 p-12 text-center transition-colors duration-200 backdrop-blur-md">
        <p className="text-gray-500 dark:text-slate-400">Nenhuma transação encontrada para este período.</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50 dark:bg-transparent border-b border-gray-100 dark:border-white/5">
            <tr>
              <th className="text-left p-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Data</th>
              <th className="text-left p-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Descrição</th>
              <th className="text-left p-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Categoria</th>
              <th className="text-left p-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Pagamento</th>
              <th className="text-left p-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-48">Valor</th>
              <th className="text-center p-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Ações</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-white/5">
            {transactions.map((transaction) => {
               ...
               const barColor = transaction.type === 'income' ? 'bg-emerald-500' : 'bg-fuchsia-500';

               return (
                <tr key={transaction.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                  <td className="p-4 text-sm text-gray-600 dark:text-slate-300 whitespace-nowrap font-medium">
                    {formatDate(transaction.date)}
                  </td>
                  <td className="p-4 text-sm font-medium text-gray-900 dark:text-white">
                    <div className="flex items-center space-x-3">
                      <div className={`p-1.5 rounded-full ${transaction.type === 'income' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-red-100 dark:bg-fuchsia-500/20 text-red-600 dark:text-fuchsia-400'}`}>
                  ...
                  <td className="p-4 text-sm text-gray-600 dark:text-slate-400">{transaction.paymentMethod}</td>
                  
                  {/* Value Column with Data Bar */}
                  <td className="p-4 text-sm font-bold align-middle">
                    <div className="flex flex-col justify-center h-full">
                      <span className={`${transaction.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-fuchsia-400'} z-10 relative mb-1`}>
