...
-- =============================================
-- 11. INTEGRAÇÃO PLUGGY (Open Finance)
-- =============================================
-- Armazena conexões do Pluggy para sincronização automática de transações
...
CREATE TABLE IF NOT EXISTS public.pluggy_connections (
    ...
    connector_name TEXT,  -- Nome do banco/cartão (ex: "Nubank", "Itaú")
    connector_logo TEXT,  -- URL do logo do conector
    account_type TEXT,    -- Tipo: credit_card, checking, savings, etc
    status TEXT DEFAULT 'active' CHECK (status IN ('active', 'error', 'updating', 'inactive')),
...
-- Comentários
COMMENT ON TABLE public.pluggy_connections IS 'Conexões Pluggy para importação automática de transações';
COMMENT ON COLUMN public.pluggy_connections.item_id IS 'ID do item no Pluggy';
COMMENT ON COLUMN public.pluggy_connections.connector_name IS 'Nome da instituição financeira';
COMMENT ON COLUMN public.pluggy_connections.status IS 'Status: active, error, updating, inactive';

-- RLS para pluggy_connections
ALTER TABLE public.pluggy_connections ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Usuários podem ver apenas suas próprias conexões Pluggy"
    ON public.pluggy_connections FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Usuários podem inserir suas próprias conexões Pluggy"
    ON public.pluggy_connections FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Usuários podem atualizar suas próprias conexões Pluggy"
    ON public.pluggy_connections FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Usuários podem deletar suas próprias conexões Pluggy"
    ON public.pluggy_connections FOR DELETE USING (auth.uid() = user_id);

-- Trigger para updated_at
CREATE TRIGGER on_pluggy_connections_updated
    BEFORE UPDATE ON public.pluggy_connections
    FOR EACH ROW
    EXECUTE FUNCTION public.handle_updated_at();

-- Coluna adicional em transactions para rastrear transações do Pluggy
-- ALTER TABLE public.transactions ADD COLUMN IF NOT EXISTS pluggy_transaction_id TEXT UNIQUE;

-- Índice para busca rápida por pluggy_transaction_id
-- CREATE INDEX IF NOT EXISTS idx_transactions_pluggy_id ON public.transactions(pluggy_transaction_id) WHERE pluggy_transaction_id IS NOT NULL;
